...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Sigo vivo", "author": "JuPitStudios" },
            { "message": "Hola como estas.", "author": "JuPitStudios" },
            { "message": "Que le dice un pato a otro pato estamos emPATADOS.", "author": "JuPitStudios" }
        ];
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then().catch(err => {
            return this.shutdown(`error en Buscando actualizaciones :<br>${err.message}`);
        });
    ...
    async dowloadUpdate() {
        ...
        this.setStatus(`Actualizacion disponible !<br><div class="download-update">Descargando</div>`);
    ...
    async maintenanceCheck() {
        config.GetConfig().then(res => {
            ...
            console.error(e);
            return this.shutdown("No se detectó conexión a internet, por favor intente de nuevo más tarde.");
        })
    }
    ...
    shutdown(text) {
        this.setStatus(`${text}<br>Detención en 5 segundos`);
        let i = 4;
        setInterval(() => {
